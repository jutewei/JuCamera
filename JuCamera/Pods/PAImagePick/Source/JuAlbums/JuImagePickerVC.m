...
@interface JuImagePickerVC ()<UINavigationControllerDelegate,UIImagePickerControllerDelegate>{
     BOOL allowsEditing;//允许编辑
}
@property(nonatomic,copy)JuImageHandle ju_handle;
@end

@implementation JuImagePickerVC
...
+(instancetype)initImagePickType:(UIImagePickerControllerSourceType)sourceType
    ...
    return [self initImagePick:allowsEditing sourceType:sourceType mediaType:JuPhotoAlbumImage  handle:handle];
}
+(instancetype)initCameraPick:(BOOL)allowsEditing
                   mediaType:(JuPhotoAlbumType)type
                      handle:(JuImageHandle)handle{
    return [self initImagePick:allowsEditing sourceType:UIImagePickerControllerSourceTypeCamera mediaType:type handle:handle];

}
+(instancetype)initImagePick:(BOOL)allowsEditing
    ...
    return [[JuImagePickerVC alloc]initImagePick:allowsEditing sourceType:sourceType mediaType:type handle:handle];
}
...
-(instancetype)initImagePick:(BOOL)allowsEditing  sourceType:(UIImagePickerControllerSourceType)sourceType  mediaType:(JuPhotoAlbumType)type handle:(JuImageHandle)handle{
    NSArray *arrTypes;
    if (type==JuPhotoAlbumImage) {
        arrTypes=@[(NSString *)kUTTypeImage];
    }else if(type==JuPhotoAlbumVideo){
        arrTypes=@[(NSString *)kUTTypeMovie];
    }else{
        arrTypes=@[(NSString *)kUTTypeImage,(NSString *)kUTTypeMovie];
    }
    JuImagePickerVC *vc= [self initImagePickType:sourceType mediaTypes:arrTypes allowsEditing:allowsEditing handle:handle];
    return vc;
}

-(instancetype)initImagePickType:(UIImagePickerControllerSourceType)sourceType mediaTypes:(NSArray *)mediaTypes allowsEditing:(BOOL)allowsEditing  handle:(JuImageHandle)handle{
    self=[super init];
    if (self) {
        self.delegate = self;
...
- (void)imagePickerController:(UIImagePickerController *)picker didFinishPickingMediaWithInfo:(NSDictionary *)info{
    ...
    if ([mediaType isEqualToString:(__bridge NSString *)kUTTypeImage]){
        ...
        if (picker.sourceType==UIImagePickerControllerSourceTypeCamera) {//拍照存图片
            if (!_isNotSave) {
                [resultImage juSaveRHAssetPhoto:^(PHAsset *Asset) {}];
            }
        }
        if (self.ju_handle) {
            self.ju_handle(resultImage);
        }
    }else if([mediaType isEqual:(__bridge NSString *)kUTTypeMovie]){
