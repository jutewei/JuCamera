...
/**无行距**/
+(CGFloat)juGetSringHeigth:(NSString *)string maxWidth:(float)width font:(CGFloat)font{
    return [self juGetAttributeSize:string maxWidth:width font:font lineSpace:0].height;
}

/**无行距**/
+(CGFloat)juGetStringWidth:(NSString *)string maxWidth:(float)height font:(CGFloat)font{
    return [self juGetAttributeSize:string maxWidth:height font:font lineSpace:0].width;
}

+(CGSize )juGetAttributeSize:(NSString *)string maxWidth:(float)width font:(CGFloat)font lineSpace:(NSInteger)space{
    ...
    if (string.length != 0) {
        ...
        if (space>0) {
            ...
            [dicAtt setValue:paragraphStyle forKey:NSParagraphStyleAttributeName];
        }
...
+(NSAttributedString *)juGetHtmlWithString:(NSString *)string{
...
-(void )juParagraphHeadIndent:(CGFloat)headIndent{
...
+(NSMutableAttributedString *)juSetPriceWithString:(NSString *)string withFont:(UIFont *)font{
    if (string) {
        ...
        if (range.location!=NSNotFound) {
            [attString addAttribute:NSFontAttributeName value:font range: NSMakeRange(range.location, string.length-range.location)];
        }
        return attString;
    }
    return nil;
...
-(void)juSetUnderLine:(NSString *)rangSring{
    if (rangSring) {
        NSRange rang=[self.string rangeOfString:rangSring];
        //        if (rang.location!=NSNotFound) {
        //        @{NSStrikethroughStyleAttributeName:@(NSUnderlinePatternSolid|NSUnderlineStyleSingle)}
        [self addAttributes:@{NSUnderlineStyleAttributeName: [NSNumber numberWithInteger:NSUnderlineStyleSingle]}  range:rang];
        //        }
    }
}
-(void )juLinkWithString:(NSString *)string link:(NSString *)link{
    if (string.length) {
        NSRange rang=[self.string rangeOfString:string];
        [self addAttributes:@{NSLinkAttributeName:link?link:string} range:rang];
    }

}
-(NSMutableParagraphStyle *)juParagraphStyle{
    NSRange range = NSMakeRange(0, self.length);
    NSDictionary *dic = [self attributesAtIndex:0 effectiveRange:&range];   // 获取该段
