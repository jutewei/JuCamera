...
+(NSString *)juSaveCacheData:(NSData *)data
    ...
    if (data) {
        ...
        NSString *path=[JuFileManage juGetFilePath:folder fileName:imageName];
        return [JuFileManage juSaveData:data path:path];
    }
    return nil;
}
...
+(NSString *)juSaveUserData:(NSData *)data
    ...
    if (data) {
        NSString *imageName=[[self juGetDateName] stringByAppendingPathExtension:suffix];
        NSString *path=[JuFileManage juGetUserPath:folder fileName:imageName];
        return [JuFileManage juSaveData:data path:path];
    }
    return nil;
}
...
+(NSString *)juGetTmpPath:(NSString *)filePath
    ...
    NSString *path=[self juCreatFilePath:NSTemporaryDirectory() filePath:filePath];
...
+(NSString *)juSaveTmpData:(NSData *)data
    ...
    if (data) {
        NSString *imageName=[[self juGetDateName] stringByAppendingPathExtension:suffix];
        NSString *filePath=[self juGetTmpPath:folder imageName:imageName];
        return [JuFileManage juSaveData:data path:filePath];
    }
    return nil;
}
...
//    NSString *dateStr=[datematter stringFromDate:date];
    return [NSString stringWithFormat:@"%@%@",dateStr,[self juGet32coding:4]];
}
